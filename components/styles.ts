// lib/styles.ts
// The complete CSS string from the original page.tsx <style> block.
// Imported and rendered once in app/page.tsx so every component
// shares the same class names without duplication.
...
export const globalStyles = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .footer-tagline { font-size: 0.84rem; color: var(--text-muted); font-weight: 300; max-width: 280px; line-height: 1.65; margin-bottom: 20px; }
  .footer-socials { display: flex; gap: 10px; }
  .footer-social-btn {
    width: 34px; height: 34px; background: var(--glass-bg);
    border: 1px solid var(--glass-border); border-radius: 8px;
    display: flex; align-items: center; justify-content: center;
    font-size: 0.88rem; text-decoration: none; color: var(--text-muted);
    transition: all 0.2s;
  }
  .footer-social-btn:hover { background: var(--glass-border); transform: translateY(-2px); }
  .footer-col h4 { font-family: var(--font-head); font-size: 0.72rem; font-weight: 700; letter-spacing: 0.12em; text-transform: uppercase; color: var(--text); margin-bottom: 16px; }
  .footer-col ul { list-style: none; display: flex; flex-direction: column; gap: 10px; }
  .footer-col a { font-size: 0.84rem; color: var(--text-muted); font-weight: 300; text-decoration: none; transition: color 0.2s; }
  ...
  .footer-legal a { font-size: 0.75rem; color: var(--text-muted); text-decoration: none; transition: color 0.2s; }
